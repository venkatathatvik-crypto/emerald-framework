...
export const Route = createFileRoute("/partner/orders/$orderId")({
  head: () => ({ meta: [{ title: "Order details — Partner" }] }),
  component: Page,
});
...
function Page() {
  ...
  const { data: schedule, isLoading: scheduleLoading } = useQuery({
    queryKey: ["partner", "order", id, "emi-schedule"],
  ...
  return (
    <DashboardShell role="partner" title="Order details">
      <Link to="/partner/orders" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-ink mb-4">
