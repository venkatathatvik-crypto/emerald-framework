...
function Page() {
  ...
  const { data: branches } = useQuery({
    queryKey: ["admin", "partner", partnerId, "branches-for-filter"],
    queryFn: () => getPartnerBranches(Number(partnerId), { size: 200 }),
    enabled: ready && partnerId !== "ALL",
  });

  const { data, isLoading, isError } = useQuery({
    queryKey: ["admin", "orders", { partnerId, branchId, from, to, page }],
    queryFn: () => listOrders({
            ...
            <TableBody>
              ...
              {orders.map((order) => (
                ...
                >
                  ...
                  <TableCell>{order.allianceCompanyName || "Direct signup"}</TableCell>
